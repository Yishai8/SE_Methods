	...
	: _console(GetStdHandle(input)), _graphics(output)
{
	...
	ShowWindow(GetConsoleWindow(), SW_MAXIMIZE);	//maximize console;
...
void EventEngine::run(Control &c)
{
	...
	{
		if (redraw)	//redraw after changes
		{
		...
		case KEY_EVENT:
		{
			...
			if (f != nullptr && record.Event.KeyEvent.bKeyDown)
			{
				...
				if (code == VK_TAB)	//change control focus - tab click
					moveFocus(c, f);
				else
					f->keyDown(code, chr);	//key pressed on control
				redraw = true;
			}
			break;
		}
		case MOUSE_EVENT:	//mouse event on a control 
		{
